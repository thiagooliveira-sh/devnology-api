...
export abstract class Database {
    ...
    static getConnectionBasic<T>(): T {
        ...
        switch (database) {
            ...
            case DatabaseTypeEnum.POSTGRES:
                if (process.env.NODE_ENV === 'development')
                    connection = { type: 'postgres', ...this.database_dev, ssl: { rejectUnauthorized: true }, extra: { ssl: true } }
                else if (process.env.NODE_ENV === 'production')
                    connection = { type: 'postgres', ...this.database_prd, ssl: { rejectUnauthorized: true }, extra: { ssl: true } }
                return connection
        }
    }
    ...
    static getConnectionOptions<T>(): T {
        ...
        switch (database) {
            ...
            case DatabaseTypeEnum.POSTGRES:
                if (process.env.NODE_ENV === 'development')
                    connection = { type: 'postgres', ...this.database_dev, schema: process.env.DATABASE_SCHEMA_DEV, ssl: { rejectUnauthorized: true }, extra: { ssl: true } }
                else if (process.env.NODE_ENV === 'production')
                    connection = { type: 'postgres', ...this.database_prd, schema: process.env.DATABASE_SCHEMA_PRD, ssl: { rejectUnauthorized: true }, extra: { ssl: true } }
                return connection
        }
